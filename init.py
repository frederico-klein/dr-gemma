...
from local_gemma.utils.benchmark import benchmark
from local_gemma.utils.config import (
...
def main():
    ...
    if args.benchmark:
        ...
        with open("instructions.txt",'r') as instructions_file:
            intro = instructions_file.read()
        while True:
            # Get input to the model
            if len(chat_history) == 0:
                user_input = intro
            elif has_starting_prompt:
            ...
            # Generate text
                ...
                if len(chat_history) == 0:
                    user_input = intro + base_prompt + user_input
